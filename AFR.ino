...
GeoLoc gpsdump(TinyGPS &gps) {
  ...
  String robot = "Received onboard GPS.\n";
  Blynk.virtualWrite(V4, robot);
...
BLYNK_WRITE(V2) {
  ...
  String remote = "Received remote GPS.\n";
  Blynk.virtualWrite(V4, remote);
...
void driveTo(struct GeoLoc &loc, int timeout) {
  ...
  if (robotLoc.lat != 0 && robotLoc.lon != 0) {
    ...
    do {
      ...
      String driving = "Distance:";
      String hh = "m.\n";
      Blynk.virtualWrite(V4, driving);
      Blynk.virtualWrite(V4, geoDistance(robotLoc, loc));
